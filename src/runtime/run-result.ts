...
import { AgentRunCancelledError } from "./run-cancellation.js";
...
export interface AgentRunSuccess { status: "completed"; stopReason: "completed"; result: AgentLoopResult; }
export interface AgentRunStopped { status: "stopped" | "failed"; stopReason: Exclude<AgentStopReason, "completed">; message: string; cause?: unknown; }
export type AgentRunResult = AgentRunSuccess | AgentRunStopped;

export function isAgentRunSuccess(result: AgentRunResult): result is AgentRunSuccess { return result.status === "completed"; }

export function toAgentRunResult(error: unknown): AgentRunStopped {
  const message = error instanceof Error ? error.message : String(error);

  if (error instanceof AgentRunCancelledError) {
    return { status: "stopped", stopReason: "cancelled", message, cause: error };
  }

  if (error instanceof AgentRunBudgetExceededError) {
    return { status: "stopped", stopReason: error.kind === "steps" ? "max_steps_reached" : "budget_exceeded", message, cause: error };
  }

  if (error instanceof TimeoutError) {
    return { status: "stopped", stopReason: "deadline_exceeded", message, cause: error };
  }
