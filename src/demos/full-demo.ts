...
export async function runFullDemo(): Promise<void> {
  ...
  for (const candidate of candidates) {
    if (shouldSaveMemory(candidate)) memoryStore.save("user_001", candidate);
  }
  ...
  console.log(
    "Candidate tools:",
    selectCandidateTools("查一下张三，明天下午提醒我跟进", catalog).map(tool => `${tool.server}:${tool.name}`)
  );
  console.log(
    "Direct MCP call:",
    await crmClient.callTool("search_customer", { keyword: "张三" })
  );
  ...
  console.log("Idempotency same id:", (first as { id: string }).id === (second as { id: string }).id);

  title("Day 6 - Human-in-the-loop guard");
  let approvalState: WorkflowState = {
